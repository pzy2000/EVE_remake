...
namespace Starfall.UI
{
    ...
    public sealed class MobileUiCoordinator : IDisposable
    {
            ...
            IWindowMetricsProvider metricsProvider)
        {
            ...
            if (screenKind == MobileScreenKind.MainMenu)
            {
                var card = contentRoot.Q<VisualElement>("menu-card");
                if (card != null)
                {
                    menuScroll = new ScrollView(ScrollViewMode.Vertical) { name = "mobile-menu-scroll" };
                    menuScroll.AddToClassList("mobile-menu-scroll");
                    menuScroll.horizontalScrollerVisibility = ScrollerVisibility.Hidden;
                    while (card.childCount > 0) menuScroll.Add(card.ElementAt(0));
                    card.Add(menuScroll);
                }
            }
#if UNITY_ANDROID && !UNITY_EDITOR
        ...
        public void Dispose()
        {
            ...
            if (menuScroll != null)
            {
                ...
                menuScroll.RemoveFromHierarchy();
                while (menuScroll.contentContainer.childCount > 0)
                    card.Add(menuScroll.contentContainer.ElementAt(0));
                menuScroll = null;
            }
            if (contentRoot != null)
            {
        ...
        private void ApplyMainMenuHinge(Rect primary, Rect secondary)
        {
            ...
            card.style.height = Mathf.Max(0f, pane.height - 2f * topInset);
            card.style.maxHeight = Mathf.Max(0f, pane.height - 2f * topInset);
