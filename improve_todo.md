...
> **实施进度（2026-09-12）**：
> - ✅ 第一批·可玩性（#1 #2 #6 #16 + #5）→ commit `d7a66c2`
> - ✅ 第二批·核心公平（#3 #4 #23-28 #29）→ commit `088e368`
> - ✅ 第三批·手感与性能（#7-13 #17-22 #31 + #44 部分）→ commit `17640ec`
> - ✅ 第五批·打磨（#44-52，#53 有意延后见下）→ commit `1374607`
> - ✅ 第四批·部分（#35 #36 #37：任务回程/卖船/声望拒代理人）→ commit `594ae84`
> - ✅ 第四批·大件（2026-09-12 第二轮，EditMode 全绿）：
>   - #32 技能系统 → `4bb8465`（7 技能、队列训练 1SP/s、存档时间戳离线补点、舰船操控 I-V 船体门槛、炮术 III 模块门槛、战斗/采矿/维修/导航加成、空间站技能页、旧档直升兼容）
>   - #33 装配约束 → `55d0678`（全船体/模块能量栅格+CPU、S/M/L 模块尺寸（重激光=中型）、EVE 式堆叠惩罚（放大器/扩展/附甲/货舱/加力）、FittingRules 供模拟+UI 共用、装配页 PG/CPU 读数）
>   - #34 NPC 生态 → `5ec07a4`（声望矩阵 ≤-5 即敌对、NPC vs NPC 交战、无武装商船巡逻低/高安并在遇袭时逃逸、警察只针对玩家、NPC 击杀不产生赏金/声望）
>   - #38 内容天花板 → `02746df`（战列巡洋舰 4 艘 @3.5M（操控 IV）、战列舰门槛升至 V、代理人等级 RNG 稳定重映射（帝國 1-2/低安 3-4/零安 4，L4 任务出战巡队）、6 品 LP 商店、故事线奖励收敛至 600k/500k）
>   - #40/#42/#43 视听与无障碍 → 本轮 M10（引擎尾焰、玩家受击火花（护盾冷色/结构暖色）、跃迁/跳跃 FOV 冲击、相机近裁剪面 0.15→0.5 + 复位平滑、敌我舰船图标形状区分、禁用态 USS、总览长按详情进战斗日志）
> - ⏳ #39 占位几何与 #41 空间站内部：需要正式美术与关卡设计投入，独立迭代，不在本轮范围
> - #53 星系重叠兜底：放宽采样会改变所有种子的随机数序列，导致现有存档失效；待 #30 存档迁移路径落地后再做
